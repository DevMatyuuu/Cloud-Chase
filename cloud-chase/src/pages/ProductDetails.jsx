...
function ProductDetails() {
  ...
  return (
    <div className='md:mb-10  pt-7 sm:pt-[10px] sl:pt-8 md:mt-16 md:pt-0 font-poppins'>
      <div className='container mx-auto w-auto bg-gradient-to-b from-gray-900 to-gray-600 md:pb-20 md:mt-10 rounded-2xl md:max-w-[1100px] max-w-[1300px] sl:max-w-[800px] sl:pb-20 md:mb-20 mb-12 pt-0  pb-20'>
       <div className='md:flex flex text-center md:gap-10 md:ml-32=0 md:w-auto md:h-auto w-32 md:pt-0 sl:pt-0 pt-10 mx-auto justify-center items-center'>
          <div className='md:relative absolute flex md:w-auto mx-auto md:mx-0 sm:w-48 w-40 md:top-0 top-80 md:h-80 sm:h-40 h-28'>
             <img src={selectedCategory.image} className='md:max-h-[400px] md:max-w-[350px] md:w-100 md:mt-10 md:ml-0 sl:pt-10 max-h-60 mx-auto ' />
          </div>
       <div className='sm:mt-52 mt-48 font-semibold md:mt-20 md:text-start'>
          <div className='text-slate-300 uppercase text-xs md:text-[14px] md:mb-4 sl:mb-3 mb-2'>{selectedCategory.categories}</div>
          <h2 className='text-white text-sm md:text-2xl md:mb-3 md:mt-2 sl:mb-5'>{selectedCategory.title}</h2>
          <div className='text-white md:text-[16px] mt-2 font-normal md:max-w-[500px] max-w-[500px] md:w-auto sl:w-auto w-[360px] text-[11px] md:text-start sl:text-center sl:ml-12 sl:max-w-[700px] text-justify md:mb-8 '>{selectedCategory.description}</div>
          <div className='flex sl:gap-5 sl:mt-5 mt-5 sm:ml-20 ml-24 md:ml-0'>
          <div className='text-white md:ml-1 md:mt-3 sl:ml-80 sl:mt-4 mt-4'>{selectedCategory.isSale ? (
      ...
      <div>
          <Button onClick={() => { addToCart(selectedCategory, paramsId); handleOpenModal(); }} className='bg-black hover:bg-slate-800 hover:scale-105 text-white md:py-4 py-2 md:px-4 px-2 md:ml-5 md:text-md sl:text-xs mt-3 sm:mt-4 sm:ml-6 ml-5 font-bold rounded-lg uppercase'>Add to cart</Button>
      </div>
    </div>
  </div>
 </div>
