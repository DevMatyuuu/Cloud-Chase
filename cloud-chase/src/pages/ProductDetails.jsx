    ...
    <div className='md:mb-10  pt-7 sm:pt-[10px] sl:pt-8 md:mt-16 md:pt-0 font-poppins'>
      <div className='container mx-auto w-auto bg-gradient-to-b from-gray-900 to-gray-600 md:pb-20 md:mt-10 rounded-2xl md:max-w-[1100px] max-w-[1300px] sl:max-w-[800px] sl:pb-20 md:mb-20 mb-12 pt-0  pb-20'>
       ...
       <div className='sm:mt-52 mt-48 font-semibold md:mt-20 md:text-start'>
          ...
          <div className='text-yellow-500 md:ml-1 md:mt-3 sl:ml-80 sl:mt-4 mt-4'>{data[0].attributes.isSale ? (
          <div className='text-yellow-500 sm:text-start'>Now: ₱{discountedPrice}<span className='grid text-slate-300'> Before: ₱{origPrice}</span></div>
          ) : (
          <div className='md:mt-3'>₱{origPrice}</div>
          )}</div>
      <div>
          <Button onClick={() => { addToCart(data, id); handleOpenModal(); }} className='bg-yellow-500 hover:bg-yellow-600 hover:text-white md:py-4 py-2 md:px-4 px-2 md:ml-5 md:text-md sl:text-xs mt-3 sm:mt-4 sm:ml-6 ml-5 text-black font-bold rounded-lg uppercase'>Add to cart</Button>
      </div>
    </div>
  </div>
 </div>
